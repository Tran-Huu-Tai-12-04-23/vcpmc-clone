...
export const resetPasswordByUserID = (userId: string, oldPassword: string, password: string) => {
    return async (dispatch: Dispatch<AuthenticateAction>) => {
        dispatch({
            type: AuthenTicationActionType.LOADING,
        });
        const result: ResponseType = await resetPassword(userId, oldPassword, password);
        ...
        dispatch({
            type: AuthenTicationActionType.LOG_ERROR,
            payload: result?.message ? result?.message : 'Đổi mật khẩu không thành công!',
        });
    };
};
