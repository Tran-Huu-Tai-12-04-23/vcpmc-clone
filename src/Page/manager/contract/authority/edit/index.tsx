...
function EditContract() {
  ...
  const { updateContractAuthority, changeCurrentContractAuthority } =
    bindActionCreators(actionContractAuthority, dispatch);
  ...
  useEffect(() => {
    id && changeCurrentContractAuthority(id);
  }, [id]);
  ...
  //
  useEffect(() => {
    if (currentContractAuthority) {
      setContractAuthorityData(currentContractAuthority);
      setFiles(currentContractAuthority.file);
      form.setFieldsValue(currentContractAuthority);
      form.setFieldValue(
        "dateEffect",
        dayjs(currentContractAuthority.dateEffect),
      );
      form.setFieldValue(
        "expireDate",
        dayjs(currentContractAuthority.expireDate),
      );
      setPersonalAuthority(currentContractAuthority.personAuthority);
      form.setFieldValue(
        "phoneNumber",
        currentContractAuthority.personAuthority.phoneNumber,
      );
      form.setFieldValue(
        "email",
        currentContractAuthority.personAuthority.email,
      );
      form.setFieldValue(
        "dateAllocated",
        dayjs(currentContractAuthority.dateAllocated),
      );
    }
  }, [currentContractAuthority, id]);
  // load users
  useEffect(() => {
    loadUsers();
  }, []);
  ...
  const handleAddContractAuthority = () => {
    if (personAuthority && id) {
      ...
      updateContractAuthority(id, newData, () => {
        router.back();
      });
    }
  };
  ...
  return (
    currentContractAuthority && (
      <div className="pr-[70px]">
        ...
        <TextHeader>
          Hợp đồng uỷ quyền bài hát - {currentContractAuthority?.numberContract}
        </TextHeader>
          ...
          <div className="flex w-full items-start justify-between gap-24">
            ...
            >
              ...
              {contractAuthorityData?.authorizedLegalEntity ===
                ...
                <>
                  <Form.Item
                    label={
                      <TextLabel className="flex h-full items-center justify-center">
                        Tên người ủy quyền:
                        <span className="pt-2 text-error">*</span>
                      </TextLabel>
                    }
                  ...
                  >
                    <AutoComplete
                      defaultValue={
                        currentContractAuthority.personAuthority?.firstName +
                        " " +
                        currentContractAuthority.personAuthority?.lastName
                      }
                      variant="outlined"
                      onSelect={(val) => {
            ...
            >
              {contractAuthorityData?.authorizedLegalEntity ===
                ...
                <>
                  ...
                  <Form.Item
                    ...
                    labelCol={{ span: 9 }}
                    rules={[
                      { required: true, message: "Nơi cấp là bắt buộc!" },
                    ]}
                  >
                    <Input
                      variant="outlined"
              ...
              {contractAuthorityData?.authorizedLegalEntity ===
                ...
                <>
                  <Form.Item
                    name="namePersonAuthority"
                    label={
                      <TextLabel
                        className="flex h-full items-center justify-center"
                        idInput="namePersonAuthority"
                      >
                        Người đại diện:
                        <span className="pt-2 text-error">*</span>
                      </TextLabel>
        ...
        <Form
          initialValues={{ ...currentContractAuthority }}
          form={form}
